...
FROM country
WHERE region ILIKE '%Afr%';
                                                                                                                SELECT * FROM country;
                                                                                                                SELECT * FROM city;
-- 3. The per capita GNP (i.e. GNP multipled by 1000000 then divided by population) of all countries in the 
-- world sorted from highest to lowest. Recall: GNP is express in units of one million US Dollars 
-- (highest per capita GNP in world: 37459.26)
SELECT ((GNP * 1000000)/POPULATION) AS "PER CAPITAL GNP" FROM COUNTRY WHERE population <> 0 ORDER BY "PER CAPITAL GNP" DESC;
...
SELECT continent, SUM(surfacearea) AS sum_surface
FROM Country
GROUP BY continent
...
SELECT lifeexpectancy, MAX(population/surfacearea) AS pop_surface
FROM country
